...
import { CancelOrderUseCase } from '@/Application/Ticketing/Orders/CancelOrderUseCase.ts';
...
Deno.test('[Unit] - CancelOrderUseCase - execute - cancels an open order', async () => {
    ...
    const useCase = new CancelOrderUseCase(unitOfWork, mockLogger);

    await useCase.execute({ orderId });

    assertEquals(testOrder.status, OrderStatus.Cancelled);
...
Deno.test('[Unit] - CancelOrderUseCase - execute - missing order does not persist', async () => {
    const unitOfWork = createMockUnitOfWork({
